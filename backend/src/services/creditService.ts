...
/**
 * Recompute a volunteer's credit score using the given database client.
 * When the client is inside an open transaction the UPDATE participates in it,
 * so callers can roll the credit recalculation into a larger atomic unit of work.
 */
export const recalculateCreditScoreWithClient = async (
  client: PoolClient,
  volunteerId: string
...
export const logCreditChangeWithClient = async (
  client: PoolClient,
  volunteerId: string,
  changeAmount: number,
  reason: string,
...
export const recalculateCreditScore = async (
  ...
  try {
    return await recalculateCreditScoreWithClient(client, volunteerId);
  } finally {
    client.release();
  }
};
